...
Modo modeless: todas as acoes de escrita sao delegadas via External Events.
"""
import random
...
from System.Windows.Controls import (CheckBox as WpfCheckBox, ComboBox,
...
from pf_models import ValueItem, ScheduleTemplateItem, NomeItem
from pf_managers import StateManager, PresetManager
from pf_helpers import get_id_value, sanitize_view_name, nome_unico, get_discipline
from pf_core import (
    ...
    get_param_guid, filter_nonempty_params
)
    ...
    def _on_create(self, sender, args):
        try:
            config = {
                ...
                "order":         ("original" if self.rbOrderOriginal.IsChecked
                                  else ("alpha" if self.rbOrderAlpha.IsChecked else "count"))
            }
...
class ParamForgeWindow(Window):
    def __init__(self, doc, uidoc, rvt_year, handlers):
        self.doc      = doc
        self.uidoc    = uidoc
        ...
        # Guardar handlers e eventos para evitar GC e para uso nos botoes
        self._handlers = handlers
        self._h_apply,     self._e_apply     = handlers["apply"]
        self._h_reset,     self._e_reset     = handlers["reset"]
        self._h_filters,   self._e_filters   = handlers["filters"]
        self._h_legend,    self._e_legend    = handlers["legend"]
        self._h_schedules, self._e_schedules = handlers["schedules"]
        self._h_isolate,   self._e_isolate   = handlers["isolate"]
        self._h_section3d, self._e_section3d = handlers["section3d"]
        self._h_pick,      self._e_pick      = handlers["pick"]

        # Apontar window_ref para self
        for h, _ in handlers.values():
            h.window_ref = self
        ...
        # Template combos/checks: {cat_name: (ComboBox, WpfCheckBox)}
    ...
    def _on_modo_selecao(self, sender, args):
        """Inicia selecao no modelo via External Event (janela e ocultada)."""
        self._e_pick.Raise()
    ...
    def _style_mode_buttons(self, modo):
        _c      = BrushConverter()
        act_bg  = _c.ConvertFrom("#7C3AED")
        act_fg  = _c.ConvertFrom("#FFFFFF")
        inact_bg = _c.ConvertFrom("#FFFFFF")
        inact_fg = _c.ConvertFrom("#374151")
        inact_bd = _c.ConvertFrom("#D1D5DB")
        t0 = Thickness(0)
        t1 = Thickness(1)

        for btn, m in [(self.btnModoSelecao, "selecao"),
                       (self.btnModoVista,   "vista"),
                       (self.btnModoProjeto, "projeto")]:
            if m == modo:
                btn.Background = act_bg;  btn.Foreground = act_fg;  btn.BorderThickness = t0
            else:
                btn.Background = inact_bg; btn.Foreground = inact_fg
                btn.BorderBrush = inact_bd; btn.BorderThickness = t1

    def set_selection(self, elements):
        """Chamado pelo PickSelectionHandler via Dispatcher.BeginInvoke."""
    ...
    def _update_params(self):
        ...
        elems = (self._sel_elements
                 if self._mode == "selecao" and self._sel_elements else None)
        raw_params = get_parameters(
            self.doc, selected_cats, self._get_active_view_id(),
            self.type_cache, elements=elems)

        self.all_params = filter_nonempty_params(
            self.doc, selected_cats, raw_params,
            self._get_active_view_id(), self.type_cache, elements=elems)
        ...
        # BUG FIX: usar objetos do ItemsSource, nao strings de outra origem
    ...
    def _on_value_dblclick(self, sender, args):
        if not self.lvValues.SelectedItem:
            return
    ...
    def _on_save_preset(self, sender, args):
        if not self.current_values:
            return
        name = forms.ask_for_string(prompt="Nome do preset:", title="Salvar Preset",
                                    default="Meu Preset")
        self._bring_to_front()
        if not name:
            return
    ...
    def _on_load_preset(self, sender, args):
        ...
        for item in self.current_values:
            if item.Value in colors_data:
                ...
                applied += 1
        self.lvValues.Items.Refresh()
        self.txtStatus.Text = "Preset '{}': {}/{} cores.".format(
            selected, applied, len(self.current_values))
    ...
    # -----------------------------------------------------------------------
    # VISUALIZAR  (todos via External Event)
    # -----------------------------------------------------------------------
    ...
    def _on_apply(self, sender, args):
        ...
        if not checked:
            ...
            self._bring_to_front()
            return
        self._h_apply.checked   = checked
        self._h_apply.all_views = bool(self.chkAllViews.IsChecked)
        self.txtStatus.Text = "Aplicando cores..."
        self._e_apply.Raise()

    def _on_reset_cores(self, sender, args):
        self._h_reset.current_values = list(self.current_values)
        self._h_reset.selected_cats  = self._get_selected_cats()
        self._h_reset.all_views      = bool(self.chkAllViews.IsChecked)
        self.txtStatus.Text = "Resetando..."
        self._e_reset.Raise()
    ...
    def _on_create_filters(self, sender, args):
        ...
        if not selected_params or not selected_cats:
            ...
            self._bring_to_front()
            return
        self._h_filters.checked              = checked
        self._h_filters.selected_param_names = selected_params
        self._h_filters.selected_cats        = selected_cats
        self._h_filters.all_views            = bool(self.chkAllViews.IsChecked)
        self.txtStatus.Text = "Criando filtros..."
        self._e_filters.Raise()
    ...
    def _on_open_legend(self, sender, args):
        ...
        legend_h = self._h_legend
        legend_e = self._e_legend

        def callback(config):
            legend_h.checked = list(checked)
            legend_h.config  = config
            legend_e.Raise()

        legend_win = LegendConfigWindow(callback, len(checked))
        legend_win.ShowDialog()
        self._bring_to_front()
        self.txtStatus.Text = "Criando legenda..."
    ...
    def _update_templates(self):
        ...
        # IMPROVEMENT: refresh do mapa a cada abertura do painel
        schedule_cat_map = get_categories_with_schedules(self.doc)
        all_schedules    = get_schedule_templates(self.doc) if ver_todos else None
        ...
        for cat in selected_cats:
            ...
            # Selecionar schedules para esta categoria
            if ver_todos:
                raw_schedules = all_schedules
            else:
                raw_schedules = schedule_cat_map.get(cat_id_val, [])
                if not raw_schedules:
                    if all_schedules is None:
                        all_schedules = get_schedule_templates(self.doc)
                    raw_schedules = all_schedules

            # IMPROVEMENT + BUG FIX: armazenar ScheduleTemplateItem
            # para recuperar ElementId sem busca por nome
            for s in raw_schedules:
                if isinstance(s, ScheduleTemplateItem):
                    cmb.Items.Add(s)
                else:
                    cmb.Items.Add(ScheduleTemplateItem(s, cat_id_val))
    ...
    def _on_create_schedules(self, sender, args):
        ...
        # Resolver templates (BUG FIX: via ScheduleTemplateItem, nao por nome)
        templates_por_cat = {}
        enabled_cats = []
        for cat in selected_cats:
            cat_name = cat.Name
            ...
            if not cmb or cmb.SelectedItem is None:
                forms.alert("Template para '{}' nao selecionado.".format(cat_name),
                            exitscript=False)
                self._bring_to_front()
                return
            # SelectedItem e ScheduleTemplateItem — pegar .Schedule diretamente
            tpl_item = cmb.SelectedItem
            template = (tpl_item.Schedule
                        if isinstance(tpl_item, ScheduleTemplateItem)
                        else tpl_item)
        ...
        # Elemento de referencia para resolucao de GUIDs
        first_elem = self.doc.GetElement(checked[0].ElementIds[0])
        if not first_elem:
            forms.alert("Elemento de referencia nao encontrado. Atualize os valores.",
                        exitscript=False)
            self._bring_to_front()
            return
        ...
        # Construir plano sem validacao previa
        # (duplicar_e_filtrar ja ignora campos nao filtráveis)
        plano_items = []
        for item in checked:
            ...
            for cat_name, template in templates_por_cat.items():
                if prefix:
        ...
        # Dialog de confirmacao (modal no thread WPF — correto)
        ...
        # Disparar External Event para criacao
        self._h_schedules.plano_final = confirm.plano_final
        self.txtStatus.Text = "Criando schedules..."
        self._e_schedules.Raise()
    ...
    def _on_preview_iso(self, sender, args):
        ...
        if not ids:
            ...
            self._bring_to_front()
            return
        self._h_isolate.element_ids = ids
        self.txtStatus.Text = "Isolando..."
        self._e_isolate.Raise()
    ...
    def _on_preview_3d(self, sender, args):
        ...
        if not ids:
            ...
            self._bring_to_front()
            return
        self._h_section3d.element_ids = ids
        self.txtStatus.Text = "Calculando Section Box..."
        self._e_section3d.Raise()
    ...
    def _restore_state(self):
        ...
        if saved_params:
            saved_set = set(saved_params)
            self._selected_params = set(p for p in self.all_params if p in saved_set)
            self._updating_params = True
            # BUG FIX: usar objetos do ItemsSource para garantir referencia correta
            for p in self.all_params:
                if p in saved_set:
                    self.lbParameters.SelectedItems.Add(p)
            self._updating_params = False
    ...
    def _on_closing(self, sender, args):
        state = {
            ...
            "colors":     {item.Value: (item.R, item.G, item.B)
                           for item in self.current_values},
            "checked":    {item.Value: item.IsChecked
                           for item in self.current_values},
        }
        old = StateManager.load()
        if "legend_config" in old:
            state["legend_config"] = old["legend_config"]
        StateManager.save(state)

        # Limpar window_ref nos handlers para evitar retencao de referencia
        for h, _ in self._handlers.values():
            try:
                h.window_ref = None
            except Exception:
                pass
