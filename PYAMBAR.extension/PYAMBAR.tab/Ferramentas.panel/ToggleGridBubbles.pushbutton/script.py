# -*- coding: utf-8 -*-
"""
Toggle Grid Bubbles v1.1
Controla visibilidade das bolhas de grid em vistas
...
def collect_valid_views():
    """
    Coleta vistas válidas para aplicar toggle de grid bubbles.
...
def apply_grid_changes_to_views(selected_views, coordinate_system, angle_tolerance, grid_states):
    """
    Aplica as mudanças de grid bubbles para múltiplas vistas.

    Args:
        selected_views: Lista de vistas selecionadas
        coordinate_system: Sistema de coordenadas escolhido
        angle_tolerance: Tolerância angular
        grid_states: Dicionário com estados dos grids {direction: visible}

    Returns:
        tuple: (success_count, failed_views)
    """
    success_count = 0
    failed_views = []
    ...
    for view in selected_views:
        try:
            # Criar CustomGrids para esta vista
            grids = CustomGrids(doc, view, coordinate_system, angle_tolerance)

            if not grids.is_valid:
                failed_views.append((view.Name, "No valid grids found"))
                continue

            # Aplicar estados dos grids
            for direction, is_visible in grid_states.items():
                action = "show" if is_visible else "hide"
                grids.toggle_bubbles_by_direction(action, direction)
    ...
    return success_count, failed_views
...
def main():
    ...
    # OPÇÃO: Usar apenas vista ativa ou múltiplas vistas?
    use_multiple_views = forms.alert(
        "Escolha o modo de operação:",
        options=["Vista Ativa Apenas", "Múltiplas Vistas"],
        cancel=True
    )

    if not use_multiple_views:
        return

    selected_views = []

    if use_multiple_views == "Múltiplas Vistas":
        ...
        # Usar apenas vista ativa
        selected_views = [active_view]

    # Usar a primeira vista para validação inicial
    first_view = selected_views[0]
    grid_collector = GridsCollector(doc, first_view)

    if not grid_collector.check_validity():
        return

    tg = DB.TransactionGroup(doc, "Toggle Grids - {} vistas".format(len(selected_views)))
    tg.Start()

    # Check if view is elevation/section, if true skip coordinate system selection
    if first_view.ViewType in ELEVATION_VIEWS:
        ...
        if window is not None:
            ...
            if window.result == "cancel":
                ...
                # Se múltiplas vistas, aplicar para as demais
                if len(selected_views) > 1:
                    # Capturar estados dos checkboxes da janela
                    grid_states = {
                        "top": window.checkboxes["top"].IsChecked if window.checkboxes["top"] else False,
                        "bottom": window.checkboxes["bottom"].IsChecked if window.checkboxes["bottom"] else False,
                        "left": window.checkboxes["left"].IsChecked if window.checkboxes["left"] else False,
                        "right": window.checkboxes["right"].IsChecked if window.checkboxes["right"] else False,
                    }

                    # Aplicar para as demais vistas (pulando a primeira)
                    remaining_views = selected_views[1:]
                    success_count, failed_views = apply_grid_changes_to_views(
                        remaining_views,
                        coordinate_system,
                        angle_tolerance,
                        grid_states
                    ...
                    total_success = success_count + 1  # +1 pela primeira vista

                    message = "Grid bubbles atualizados!\n\n"
                    message += "Total de vistas: {}\n".format(total_views)
                    message += "Sucessos: {}\n".format(total_success)
                    message += "Falhas: {}".format(len(failed_views))

                    if failed_views:
                        message += "\n\nVistas com falha:\n"
                        for view_name, error in failed_views[:5]:  # Mostrar apenas 5
                            message += "- {}: {}\n".format(view_name, error)
                        if len(failed_views) > 5:
                            message += "... e mais {} vistas".format(len(failed_views) - 5)

                    forms.alert(message, title="Resultado")
