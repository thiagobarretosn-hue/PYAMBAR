...
"""
Move Connect v4.2
Move e conecta elementos MEP com rotacao automatica
...
DESCRICAO:
Move e conecta elementos MEP automaticamente COM ROTACAO.
Move o primeiro elemento ate o segundo, conectando e ajustando orientacao.
Baseado em pyRevitMEP ConnectTo (Cyril Waechter).

WORKFLOW:
1. Execute o script
2. Selecione o elemento ALVO (fixo)
3. Selecione o elemento a ser MOVIDO
4. O elemento movido se conecta ao alvo com rotacao automatica

DIFERENCA DO "CONNECT NO ROTATE":
- Connect No Rotate: Mantem orientacao original (apenas translacao)
- Move Connect: Ajusta rotacao para alinhar conectores (translacao + rotacao)
"""
...
def main():
    ...
    try:
        ...
        # Conectar usando snippet (com rotacao)
        with revit.Transaction("Conectar Movendo"):
            ...
            if not success:
                forms.alert(
                    ...
                    "Verifique se os elementos podem ser movidos/rotacionados.",
                    title="Erro",
                    warn_icon=True
                )
